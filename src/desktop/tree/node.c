...
// Returns NULL on fail.
struct e_view_container* e_view_container_try_from_e_node_desc(struct e_node_desc* node_desc)
{
    assert(node_desc);

    if (node_desc->type != E_NODE_DESC_VIEW_CONTAINER)
        return NULL;

    return (struct e_view_container*)node_desc->data;
}
